...
**已实现**：可选 `enableVision` 开关（默认关闭）；开启后，DOM 恢复动作（重试快照/滚动）耗尽时，恢复引擎选取 `vision_locate`——`bridge.capture()` 截取当前页 → 视觉 LLM 确认目标是否可见 → 可见则重试快照，不可见则正常降级失败。优先级位于 DOM 动作之后、最终放弃之前。
