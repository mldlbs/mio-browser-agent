...
> 来自实机回归测试发现的稳定性问题与修复，直接提升任务完成率。版本区间 v0.1.14 - v0.1.36。
...
| data:/chrome:// 页面截图报误导的「无权限」 | `capture` 区分真实原因（浏览器限制 data:/chrome:// 页面） | v0.1.32 |
| 正常 .com 页面被误判「内容已删除」（w3schools 误报） | 删除资源检测需真实 /removed/ 或 /deleted/ 路径段 | v0.1.33 |
| find_by_vision 坐标点击报模糊的 no element | `clickAt` 越界检测 → `CLICK_OUT_OF_VIEWPORT` + 恢复链 | v0.1.34 |
| 大页面每轮快照 token 爆炸（544 元素全量渲染） | `snapshotToLines` 截断前 60 元素 + 截断提示（token 降 ~90%） | v0.1.35 |
| 长任务 history 硬裁丢「做了什么」上下文 | 语义压缩：trim 时注入已完成步骤 `[历史摘要]` | v0.1.36 |
